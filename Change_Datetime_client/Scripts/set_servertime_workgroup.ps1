...
Set-Variable -Name SCRIPTARGS_BASE -Value "-Command `"C:\scripts\Change_datetime\change_datetime_workgroup.ps1" -Option constant
...
# -------- #
    ...
    New-CimSession -Name $name -ComputerName $computer_name -Credential $cred -Authentication Basic -ErrorAction Stop
...
switch ($Mode) {
    ...
    "Return" {
        $now = Get-Date
        $script_args = $SCRIPTARGS_BASE + " -mode $Mode -year $($now.Year) -month $($now.Month) -day $($now.Day) -hour $($now.Hour) -minute $($now.Minute)`""
        try {
            $action = createNewScheduledTaskAction -execute $PSPATH -arguments $script_args -working_dir $WORKING_DIR
        }
        catch {
