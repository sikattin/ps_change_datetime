...
function ReturnDate {
    # DateTime Synchronization to DomainController sets "NT5DS"
    try {
        $result = Invoke-Expression (Join-Path $working_dir sync_on.ps1)
    }
    catch {
        $logger.error.Invoke("Failed to do synchronized setting to DomainController.")
        throw
    }
    if ($result) {
        $logger.info.Invoke("DateTime Synchronization type sets NT5DS. start time synchronization.")
    }
    $result
}
